...
namespace BethanysPieShop.InventoryManagement
{
    ...
    {
        private static List<RegularProduct> inventory = new();
        ...
        internal static void InitializeStock()//Mock implementation
        {
            ...
            //    Product p2 = new Product(2, "Cake decorations", "Lorem ipsum", new Price() { ItemPrice = 8, Currency = Currency.Euro },20, UnitType.PerItem);
            //    Product p3 = new Product(3, "Strawberry", "Lorem ipsum", new Price() { ItemPrice = 3, Currency = Currency.Euro }, 10, UnitType.PerBox);
            //    inventory.Add(p1);
            //    inventory.Add(p2);
            //    inventory.Add(p3);
        }
        ...
        private static void ShowDetailsAndUseProduct()
        {
            ...
            {
                RegularProduct? selectedProduct = inventory.Where(p => p.Id == int.Parse(selectedProductId)).FirstOrDefault();
        ...
        {
            List<RegularProduct> lowOnStockProducts = inventory.Where(p => p.IsBelowStockThreshold).ToList();
        ...
        private static void ShowFulfilledOrders()
        {
            ...
            foreach (var order in orders)
            {
                if (!order.Fulfilled && order.OrderFulfilmentDate < DateTime.Now) // fulfil the order
                {
                    ...
                    {
                        RegularProduct? selectedProduct = inventory.Where(p => p.Id == orderItem.ProductId).FirstOrDefault();
                        if (selectedProduct != null)
        ...
        {
            Console.WriteLine($"Enter the new stock treshold (current value: {RegularProduct.stockThreshold}). This applies to all products!");
            Console.Write("New value: ");
            int newValue = int.Parse(Console.ReadLine() ?? "0");
            RegularProduct.stockThreshold = newValue;
            Console.WriteLine($"New stock treshold set to {RegularProduct.stockThreshold}");
        ...
        private static void ShowCreateNewProduct()
        {
            ...
            RegularProduct? newProduct = null;
            ...
            {
                case "1":
                    newProduct = new RegularProduct(newId, name, description, new Price() { ItemPrice = price, Currency = currency }, maxInStock, unitType);
                    break;
                ...
                case "3":
                    ...
                    newProduct = new FreshProduct(newId++, name, description, new Price() { ItemPrice = price, Currency = currency }, maxInStock, unitType);
            ...
            if (selectedProductId != null)
            {
                ...
                if (selectedProduct != null)
                {
                    ...
                    if (p != null)
                    {
                        ...
                        inventory.Add((RegularProduct)p);
                    }
                }
            }
            else
